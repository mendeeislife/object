...
class Shape {
   ...
   public:
    static int shapeCount;  // Обьект тоолох статик хувьсагч зарлах
    void setName(char *customName) {
        this->name = new char[strlen(customName) + 1];
    ...
    Shape(char *customName) {
        this->name = new char[strlen(customName) + 1];
        strcpy(this->name, customName);
        shapeCount++;  // Тоолуур нэмэгдүүлэх
    ...
    ~Shape() {
        delete[] name;
        shapeCount--;  // Тоолуур хорогдуулах
...
class Circle : public TwoDShape {
   public:
    ...
    void print() {
        ...
        cout << "Toirgiin tuv: " << this->x << " " << this->y << endl;
        cout << "Radius: " << this->r << endl;
        cout << "Talbai: " << this->findS() << endl;
        cout << "Toirgiin urt " << this->findP() << endl
             << endl;
    }
};
...
int main() {
    ...
    // Эрэмблэлт хийхээс өмнөх дүрсийн талбай периметер дэлгэцлэх
    cout << endl << "Before Sorting" << endl << "Perimeters: ";
    for (int i = 0; i < n; i++)
        cout << td[i]->findP() << " ";
    cout << endl << "Areas: ";
    for (int i = 0; i < n; i++)
    ...
    cout << "After Sorting" << endl << "Perimeters: ";
    

    // Периметрээр эрэмблэлт хийх
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++) {
            if (td[i]->findP() > td[j]->findP()) {
                temp = td[i];
                td[i] = td[j];
                td[j] = temp;
            }
    ...
    // Эрэмблэлт хийсний дараах дүрсийн периметер дэлгэцлэх
    for (int i = 0; i < n; i++)
        cout << td[i]->findP() << " ";

    // Талбайгаар эрэмблэлт хийх
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++) {
    ...
    // Эрэмблэлт хийсний дараах дүрсийн талбай дэлгэцлэх
    cout << endl << "Areas: ";
    for (int i = 0; i < n; i++)
        cout << td[i]->findS() << " ";
    cout << endl << endl;
